...
- 点击页签 → 切换 `activeTabId`；点击控件 → 选中控件并滚动定位到画布上对应位置
- 分组行是**结构标签，不是选择目标**：它只把分组名摊开作导航层级，点击它不改变任何选中态。
  分组级操作（改名 / ±列 / 复制 / 删除 / 批量尺寸）在画布的**悬浮编辑条**上，由 hover/focus 驱动
  > **执行期修订（2026-09-16，最终全分支审查）**：原文写「点击分组 → 选中分组」，该能力**降级、不实现**。
  > 理由：「分组选中态」在本设计里没有任何已定义行为——分组级操作在悬浮编辑条里已各有入口，
  > 凭空加一个选中态等于发明 spec 未描述的行为，且会与编辑条形成两套并行的分组操作入口。
  > 代码里 `selectedGroup` 零命中即此裁决的结果（分组行的 `cursor` 也已改回 `default`，
  > 不再用手型承诺一个不存在的可点击性）。
- 与画布选中态双向同步（当前选中项高亮）
- 需处理长文档的滚动与折叠
